...
# curl -LO https://raw.githubusercontent.com/andrephilipsson/dotfiles/main/install/arch-linux-install.sh
...
PARTUUID=\$(lsblk /dev/sda2 -o PARTUUID -d -n)

efibootmgr --disk /dev/sda --part 1 --create --label "Arch Linux LTS" --loader /vmlinuz-linux-lts --unicode "cryptdevice=PARTUUID=\${PARTUUID}:cryptroot root=/dev/vg/root rw initrd=\initramfs-linux-lts.img" --verbose
efibootmgr --disk /dev/sda --part 1 --create --label "Arch Linux" --loader /vmlinuz-linux --unicode "cryptdevice=PARTUUID=\${PARTUUID}:cryptroot root=/dev/vg/root rw initrd=\initramfs-linux.img" --verbose
